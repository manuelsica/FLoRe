...
int main(int argc, char* argv[]) 
{
    ...
    index_fwd.reserve(reads.size() * 4);
    index_rev.reserve(reads.size() * 4);
    ...
    size_t chunk = (total / num_threads) ? (total / num_threads) : 1;
    ...
    for (unsigned int th = 0; th < num_threads; th++) {
        ...
        pool.emplace_back(worker, idx, endIdx);
        idx = endIdx;
    }
    if (idx < total) {
        worker(idx, total);
    ...
    ofstream jf(json_filename);
    if (!jf.is_open()) {
        cerr << "Errore apertura file " << json_filename << "\n";
    } else {
        jf << "[\n";
    ...
    async_log("Memoria usata (peak): " + to_string(mem_kb) + " KB\n");
